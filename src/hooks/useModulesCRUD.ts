...
export function useFields(moduleId: string) {
  ...
  const fetchFields = useCallback(async () => {
    ...
    if (isUsingMySQL()) {
      ...
      const { data, error } = await supabase.from('module_fields').select('*').eq('module_id', moduleId).order('order_index');
      if (!error && data) setFields(data as unknown as DbField[]);
    }
    setLoading(false);
  }, [moduleId]);
  ...
  const createField = useCallback(async (input: {
    ...
    if (isUsingMySQL()) {
      ...
      const { data, error } = await supabase.from('module_fields').insert(payload).select().single();
      if (!error && data) {
        const field = data as unknown as DbField;
        setFields(prev => [...prev, field]);
        return field;
